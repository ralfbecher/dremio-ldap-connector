...
/**
 * A ResultSet that returns LDAP object classes as table metadata rows.
 * Used to expose configured object classes as tables in Dremio.
 */
public class ObjectClassResultSet implements ResultSet {
    private static final String[] COLUMNS = {"TABLE_CAT", "TABLE_SCHEM", "TABLE_NAME", "TABLE_TYPE", "REMARKS",
    ...
    public boolean next() throws SQLException {
        if (currentIndex < objectClasses.size() - 1) {
            currentIndex++;
            return true;
        }
        return false;
    ...
    public String getString(int columnIndex) throws SQLException {
        if (currentIndex < 0 || currentIndex >= objectClasses.size()) {
            throw new SQLException("No current row");
        }
        switch (columnIndex) {
            case 1: return null;  // TABLE_CAT
            case 2: return null;  // TABLE_SCHEM
            case 3: return objectClasses.get(currentIndex);  // TABLE_NAME
            case 4: return "TABLE";  // TABLE_TYPE
            case 5: return "LDAP Object Class";  // REMARKS
            default: return null;
        }
    }
    ...
    public ResultSetMetaData getMetaData() throws SQLException {
        return new ObjectClassResultSetMetaData();
    }
    ...
    @Override public boolean isAfterLast() throws SQLException { return currentIndex >= objectClasses.size(); }
    @Override public boolean isFirst() throws SQLException { return currentIndex == 0; }
    @Override public boolean isLast() throws SQLException { return currentIndex == objectClasses.size() - 1; }
    @Override public void beforeFirst() throws SQLException { currentIndex = -1; }
    @Override public void afterLast() throws SQLException { currentIndex = objectClasses.size(); }
    @Override public boolean first() throws SQLException { if (objectClasses.isEmpty()) return false; currentIndex = 0; return true; }
    @Override public boolean last() throws SQLException { if (objectClasses.isEmpty()) return false; currentIndex = objectClasses.size() - 1; return true; }
    ...
    /**
     * Metadata for ObjectClassResultSet.
     */
    private static class ObjectClassResultSetMetaData implements ResultSetMetaData {
        ...
        @Override public String getColumnLabel(int column) throws SQLException { return COLUMNS[column - 1]; }
        @Override public String getColumnName(int column) throws SQLException { return COLUMNS[column - 1]; }
